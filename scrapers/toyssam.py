"""
scrapers/toyssam.py
NEW ARRIVALS scraper for toyssam.com (Shopify).
...
BASE_URL = "https://www.toyssam.com"
REQUEST_TIMEOUT = 15
PAGE_LIMIT = 250


class ToysSamScraper(BaseScraper):
    """
    Fetches all products from a toyssam.com Shopify collection via JSON API.
    ...
    def _extract_collection_handle(url: str) -> str:
        """Extract collection handle from URL like /collections/hotwheels."""
    ...
    def scrape_listing(self, page: dict) -> ListingResult:
        ...
        while True:
            ...
            try:
                logger.info(f"[ToysSam] Fetching: {api_url}")
                ...
                logger.error(f"[ToysSam] Request error on page {page_num}: {e}")
                return ListingResult(page_name=name, page_url=url,
                                     products=all_products, error=str(e))
            except Exception as e:
                logger.exception(f"[ToysSam] Unexpected error: {e}")
                return ListingResult(page_name=name, page_url=url,
                                     products=all_products, error=str(e))

        logger.info(f"[ToysSam] Found {len(all_products)} products in '{handle}'")
